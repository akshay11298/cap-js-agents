...
async function hasScheduledCleanupInNext24h(serviceName, ttlMs, now = Date.now()) {
  const nextCleanupWindowStart = new Date(now).toISOString()
  const nextCleanupWindowEnd = new Date(now + ttlMs + MS_OF_A_DAY).toISOString()
  const existingMessage = await SELECT.one.from(OUTBOX_MESSAGES).where`
    ...
    timestamp > ${nextCleanupWindowStart} and 
    timestamp <= ${nextCleanupWindowEnd}
    `
...
export async function triggerCleanup(serviceName) {
  ...
  const now = Date.now()
  if (await hasScheduledCleanupInNext24h(serviceName, ttlMs, now)) {
    serviceMap.set(serviceName, now)
    LOG.debug(
      `Skip scheduling deletion of tasks for ${serviceName} because a cleanupTasks job is already scheduled within the next 24h cleanup window.`,
    )
    return
  }
  serviceMap.set(serviceName, now)
